    ...
    class WPClever_Woovr_Backend {
        ...
        function process_product_meta( $post_id ) {
            if ( isset( $_POST['_woovr_active'] ) ) {
                update_post_meta( $post_id, '_woovr_active', sanitize_text_field( wp_unslash( $_POST['_woovr_active'] ) ) );
            } else {
        ...
        function save_variation_settings( $post_id ) {
            if ( isset( $_POST['woovr_name'][ $post_id ] ) ) {
                update_post_meta( $post_id, 'woovr_name', sanitize_text_field( wp_unslash( $_POST['woovr_name'][ $post_id ] ) ) );
            } else {
                delete_post_meta( $post_id, 'woovr_name' );
            }

            if ( isset( $_POST['woovr_image'][ $post_id ] ) ) {
                update_post_meta( $post_id, 'woovr_image', sanitize_url( wp_unslash( $_POST['woovr_image'][ $post_id ] ) ) );
            } else {
                delete_post_meta( $post_id, 'woovr_image' );
            }

            if ( isset( $_POST['woovr_image_id'][ $post_id ] ) ) {
                update_post_meta( $post_id, 'woovr_image_id', sanitize_text_field( wp_unslash( $_POST['woovr_image_id'][ $post_id ] ) ) );
            } else {
